...
bool InoutImpl::getExoticMultiplexer() const
{
  unsigned access_position = 0;
  unsigned internal_id = io_index;
  eb_data_t readExoticMultiplexer;
  etherbone::Cycle cycle;
  ...
  if (io_channel == IO_CFG_CHANNEL_GPIO)
  {
    if (access_position == 0) { cycle.read(io_control_addr+eGPIO_Sel_Set_low,  EB_DATA32, &readExoticMultiplexer); }
    else                      { cycle.read(io_control_addr+eGPIO_Sel_Set_high, EB_DATA32, &readExoticMultiplexer); }
  }
  else if (io_channel == IO_CFG_CHANNEL_LVDS)
  {
    if (access_position == 0) { cycle.read(io_control_addr+eLVDS_Sel_Set_low,  EB_DATA32, &readExoticMultiplexer); }
    else                      { cycle.read(io_control_addr+eLVDS_Sel_Set_high, EB_DATA32, &readExoticMultiplexer); }
  }
  else                        { throw Gio::DBus::Error(Gio::DBus::Error::INVALID_ARGS, "IO channel unknown!"); }
  cycle.close();

  readExoticMultiplexer = readExoticMultiplexer&(1<<internal_id);
  readExoticMultiplexer = readExoticMultiplexer>>internal_id;

  if (readExoticMultiplexer) { return true; }
  else                    { return false; }
}

void InoutImpl::setExoticMultiplexer(bool val)
{
  unsigned access_position = 0;
  unsigned internal_id = io_index;
  etherbone::Cycle cycle;
  ...
  if (io_channel == IO_CFG_CHANNEL_GPIO)
  {
    if (val)
    {
      if (access_position == 0) { cycle.write(io_control_addr+eGPIO_Sel_Set_low,  EB_DATA32, (1<<internal_id)); }
      else                      { cycle.write(io_control_addr+eGPIO_Sel_Set_high, EB_DATA32, (1<<internal_id)); }
    }
    else
    {
      if (access_position == 0) { cycle.write(io_control_addr+eGPIO_Sel_Reset_low,  EB_DATA32, (1<<internal_id)); }
      else                      { cycle.write(io_control_addr+eGPIO_Sel_Reset_high, EB_DATA32, (1<<internal_id)); }
    }
  }
  else if (io_channel == IO_CFG_CHANNEL_LVDS)
  {
    if (val)
    {
      if (access_position == 0) { cycle.write(io_control_addr+eLVDS_Sel_Set_low,  EB_DATA32, (1<<internal_id)); }
      else                      { cycle.write(io_control_addr+eLVDS_Sel_Set_high, EB_DATA32, (1<<internal_id)); }
    }
    else
    {
      if (access_position == 0) { cycle.write(io_control_addr+eLVDS_Sel_Reset_low,  EB_DATA32, (1<<internal_id)); }
      else                      { cycle.write(io_control_addr+eLVDS_Sel_Reset_high, EB_DATA32, (1<<internal_id)); }
    }
  }
  cycle.close();
  ExoticMultiplexer(val);
}
